...
suite('Dream Extension Test Suite', () => {
    ...
    test('Completion provider should work', async () => {
        ...
        if (completions && completions.items) {
            const consoleItems = completions.items.filter(item => 
                item.label.toString().includes('Console')
            );
    ...
    test('Hover provider should work', async () => {
        ...
        if (hovers && hovers.length > 0) {
            ...
            if (typeof hoverContent === 'string') {
                assert.ok(hoverContent.includes('int') || hoverContent.includes('integer'), 
                         'Hover should contain type information');
            } else if (hoverContent && 'value' in hoverContent) {
                assert.ok(hoverContent.value.includes('int') || hoverContent.value.includes('integer'), 
                         'Hover should contain type information');
            }
        }
    });
