...
suite('Dream Language Server Test Suite', () => {
    ...
    test('Should generate keyword completions', () => {
        ...
        assert.strictEqual(completionItems.length, keywords.length, 'Should have completion for each keyword');
        ...
        assert.strictEqual(funcCompletion.kind, node_1.CompletionItemKind.Keyword);
    ...
    test('Should generate Console function completions', () => {
        ...
        assert.strictEqual(writeLineFunc.kind, node_1.CompletionItemKind.Method);
        assert.ok((_a = writeLineFunc.detail) === null || _a === void 0 ? void 0 : _a.includes('void'), 'WriteLine should return void');
        const readLineFunc = consoleFunctions.find(f => f.label === 'Console.ReadLine');
        assert.ok(readLineFunc, 'Should have ReadLine function');
        assert.ok((_b = readLineFunc.detail) === null || _b === void 0 ? void 0 : _b.includes('string'), 'ReadLine should return string');
    });
    /**
    ...
    test('Should provide builtin type information', () => {
        ...
        assert.ok(intInfo === null || intInfo === void 0 ? void 0 : intInfo.includes('32-bit integer'), 'int info should mention 32-bit integer');
        const moduleInfo = getBuiltinInfo('module');
        assert.ok(moduleInfo === null || moduleInfo === void 0 ? void 0 : moduleInfo.includes('Module declaration'), 'module info should mention declaration');
    });
    /**
    ...
    test('Should extract words correctly at cursor position', () => {
        const getWordAtPosition = (text, line, character) => {
        ...
        for (const testCase of testCases) {
            ...
            assert.strictEqual(result, testCase.expected, `Word extraction failed for "${testCase.text}" at ${testCase.char}`);
        }
    });
    /**
